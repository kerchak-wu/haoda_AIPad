...
- 通过 MQTT 订阅 topic/city 主题，收到数字 N 切换到第 N 个城市
"""
...
# ---------- MQTT 配置 ----------
MQTT_BROKER = "192.168.2.251"
MQTT_PORT = 1883
MQTT_TOPIC = "topic/city"
# MQTT 回调写入的待切换城市下标（0-based），主循环读取后清空
mqtt_pending_idx = None
...
def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        print(f"MQTT 已连接 {MQTT_BROKER}:{MQTT_PORT}，订阅主题 {MQTT_TOPIC}")
        client.subscribe(MQTT_TOPIC)
    else:
        print(f"MQTT 连接失败，返回码: {rc}")

def on_message(client, userdata, msg):
    global mqtt_pending_idx
    try:
        payload = msg.payload.decode('utf-8', errors='replace').strip()
        city_num = int(payload)
        # 收到数字 1 显示第一个城市，以此类推
        if 1 <= city_num <= len(CITIES_CONFIG):
            mqtt_pending_idx = city_num - 1
            print(f"MQTT 收到 {city_num}，切换到 {CITIES_CONFIG[city_num - 1]['name']}")
        else:
            print(f"MQTT 收到越界编号 {city_num}（有效范围 1-{len(CITIES_CONFIG)}）")
    except ValueError:
        print(f"MQTT 收到非数字消息: {msg.payload!r}")

def connect_mqtt():
    client = mqtt_client.Client(
        client_id='haoda_weather',
...
def main():
    ...
    pygame.display.set_caption("MQTT 城市天气订阅展示")
    ...
    # ---------- 初始化 MQTT ----------
    mqtt_client_inst = connect_mqtt()
    if mqtt_client_inst:
        mqtt_client_inst.loop_start()  # 后台线程处理 MQTT 网络事件
    else:
        print("MQTT 未连接，仍可手动点击/按键切换城市")
    ...
    while running:
        ...
        # ---------- 处理 MQTT 下发的城市切换 ----------
        if mqtt_pending_idx is not None:
            if 0 <= mqtt_pending_idx < len(CITIES_CONFIG) and selected_idx != mqtt_pending_idx:
                selected_idx = mqtt_pending_idx
            mqtt_pending_idx = None
        ...
        # ===== 顶部标题 =====
        # 拉开 Y 轴距离，消除重叠
        draw_text(screen, "MQTT 城市天气订阅展示", font_title, WHITE, (WIDTH // 2, 25), anchor="midtop")
        ...
        # 底部提示
        draw_text(screen, "点击城市切换  |  ↑↓ 切换  |  MQTT topic/city 收数字切换  |  ESC 退出", font_small, (220, 220, 220), (WIDTH // 2, HEIGHT - 28), anchor="center", shadow=False)
