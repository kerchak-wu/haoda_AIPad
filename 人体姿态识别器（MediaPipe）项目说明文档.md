...
| USB 外接摄像头 | 接 ESP32 扩展板 USB 口，分辨率 640×480，30fps |
| 显示屏 | 1920×1080 |

### 摄像头连接说明

- 摄像头插入 ESP32 扩展板的 USB 口
...
├── logs/                         # 日志目录（运行时自动创建）
│   └── 人体姿态识别器_YYYYMMDD.log   # 按日期命名的运行日志（追加模式）
└── 其他范例程序与学习手册
```
...
- `logs/人体姿态识别器_YYYYMMDD.log`：运行日志，记录所有 `print` 输出（含异常堆栈），用于事后排查问题。每次运行追加写入，文件名按当天日期生成。
...
```python
# 日志文件：logs/人体姿态识别器_YYYYMMDD.log，追加模式，块缓冲
_LOG_DIR = 'logs'
_LOG_FILE = os.path.join(_LOG_DIR, '人体姿态识别器_%s.log' %
...
- **目录与命名**：`logs/` 目录、`人体姿态识别器_YYYYMMDD.log`，符合项目日志规范
...
> 退出按钮位于标题栏右上角（`WIDTH-280, 20, 240×70`），底部栏仅显示状态信息。
...
**A**：日志文件位于程序同目录下的 `logs/` 文件夹，文件名格式为 `人体姿态识别器_YYYYMMDD.log`（如 `人体姿态识别器_20260811.log`）。程序崩溃或异常退出后，可在日志文件中查看完整的 `print` 输出和异常堆栈，用于事后排查。同一天多次运行会追加到同一文件，以 `======== 时间 运行开始 ========` 分隔。
...
5. **日志输出不可或缺**：嵌入式设备上程序异常退出时控制台输出可能丢失，必须用 `_TeeStdout` 将 `print` 双写到文件，事后才能排查。日志文件按日期命名、追加模式、块缓冲，避免高频写入阻塞主循环。
6. **ARM 上避免 std() 帧检测**：`frame.std()` 在 ARM 设备上计算开销过大，应改用 `gray.mean()` 检测全黑/全白帧，在保证基本有效性的前提下降低 CPU 占用。
7. **退出按钮位置一致性**：退出按钮统一置于标题栏右上角，不设在右下角，符合项目界面规范，方便触屏操作�
