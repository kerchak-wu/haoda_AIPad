...
人脸识别触发视频播放程序
================================================================
参考：人脸学习.py（FaceEngine 引擎）与 人脸学习与识别项目文档.md

功能：
  实时识别摄像头画面中的人脸，识别到指定人员后自动播放对应视频：
    识别到 zhao（赵） → 播放 1.mp4
    识别到 qian（钱） → 播放 2.mp4
    识别到 luo （罗） → 播放 3.mp4
  ...
  - 视频播放结束或按 ESC 返回识别模式
  - 同一人播放后不会立即重复触发，需离开画面再出现才会再次播放

前置条件：
  1. 已通过 人脸学习.py 学习登记了 zhao / qian / luo 三人人脸
     （姓名可用拼音 zhao/qian/luo 或中文 赵/钱/罗，程序自动匹配）
  2. 当前目录下存在 1.mp4 / 2.mp4 / 3.mp4 视频文件
...
from 人脸学习 import FaceEngine, find_chinese_font, draw_text, Button
...
# 人脸姓名 -> 视频文件 映射（拼音 / 中文 均可，匹配时不区分大小写）
...
# 识别 / 触发参数
RECOG_THRESHOLD = 75.0       # LBPH 距离阈值（与人脸学习.py 一致）
...
FACE_BOX_OTHER = (180, 180, 180)


# ===========================================================================
# 摄像头打开（复用人脸学习.py 的探测逻辑：MJPG + 超时 + 雪花检测）
# ===========================================================================
...
def main():
    ...
    pygame.display.set_caption("人脸识别触发视频播放")
    ...
    # ----- 人脸引擎 -----
    print("人脸引擎初始化中...")
    engine = FaceEngine(threshold=RECOG_THRESHOLD)
    print("人脸库已加载：共 {} 人".format(engine.count()))
    for fid, info in engine.list_faces():
        print("  ID={}  姓名={}".format(fid, info.get("name")))

    # 检查视频文件
    print("检查视频文件（目录 {}）：".format(VIDEO_DIR))
    for vf in sorted(set(FACE_VIDEO_MAP.values())):
    ...
    confirm_name = None
    confirm_count = 0
    last_played_name = None     # 刚播放过的人；其在画面中时不重复触发
    last_status = "等待识别人脸..."
    ...
    def start_video(path, name):
        ...
        # 清掉旧的识别结果，避免返回识别模式后人脸框还停留在暂停前的位置
        with recog_lock:
            latest_recog = []
    ...
    def stop_video():
        ...
        with recog_lock:
            latest_recog = []
        # 注意：last_played_name 保留，直到该人离开画面才清除，避免立即重复触发
    ...
    while running:
        ...
        if mode == MODE_RECOGNIZE:
            ...
            # 已识别且映射了视频的人脸
            matched = [(b, fid, n, c) for (b, fid, n, c) in recog_results
            ...
            if in_cooldown:
                ...
                box, fid, name, conf = max(triggerable,
                                           key=lambda r: r[0][2] * r[0][3])
                ...
                if recog_results:
                    last_status = "检测到人脸，但未匹配到 zhao/qian/luo"
                    last_status_color = DIM_TEXT
                else:
                    last_status = "等待识别人脸..."
                    last_status_color = DIM_TEXT
        ...
        # 标题
        draw_text(screen, "人脸识别触发视频播放", font_title, TEXT_COLOR,
        ...
        if mode == MODE_VIDEO:
            ...
            # ----- 识别界面 -----
            draw_text(screen,
                      "实时识别中 — 正对摄像头，识别到 zhao/qian/luo 自动播放视频",
            ...
            if cam_surf is not None and frame is not None:
                ...
                for (bx, by, bw, bh), face_id, name, conf in recog_results:
                    ...
                    if not name:
                        color = FACE_BOX_UNKNOWN
                    elif mapped:
                        color = FACE_BOX_KNOWN
                    else:
                        color = FACE_BOX_OTHER
                    pygame.draw.rect(screen, color, (rx, ry, rw, rh), 3, border_radius=6)
                    if name and mapped:
                        tag = "{} -> {}".format(name, lookup_video(name))
                    elif name:
                        tag = name
                    else:
                        tag = "未知"
            ...
            # 触发规则提示
            draw_text(screen, "触发规则：zhao -> 1.mp4    qian -> 2.mp4    luo -> 3.mp4",
